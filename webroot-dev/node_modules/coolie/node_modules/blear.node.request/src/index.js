...
var url2 = require('blear.utils.url');
...
var isEmptyObject = function (obj) {
    return typeis.Null(obj) || typeis.Undefined(obj);
};
...
var Stream = Class.ify(stream.Stream);
var Request = Stream.extend({
    ...
    debug: function (event, msg) {
        ...
        console.log(console.pretty(the._options.method + ' ' + the._urlList[the._urlList.length - 1], 'magenta'));
    ...
    _buildRequestOptions: function () {
        ...
        if (!isEmptyObject(options.browser)) {
            ...
            collection.each(browserHeaders, function (key, val) {
                if (typeis.String(val)) {
                    ...
                    switch (key) {
                        ...
                        case 'cookie':
                            var cookieList = [];
                            collection.each(the._cookies, function (key, val) {
        ...
        object.assign(ret.headers, options.headers);
    ...
    _buildRequestHeaders: function (requestOptions) {
        ...
        if (the._forms.length) {
            ...
            object.assign(requestOptions.headers, streamHeaders);
        } else {
            var requestBody = options.body;

            if (object.isPlain(options.body)) {
                try {
    ...
    _buildCookies: function () {
        ...
        collection.each(cookies, function (index, cookieString) {
    ...
    _receiveResponse: function () {
        ...
        var callbackResponse = fun.once(function () {
            if (the._finished) {
                return;
            }
        ...
        var callbackError = fun.once(function (err) {
            if (the._finished) {
                return;
            }
 ...
 * @returns {Request}
 */
request.post = function (url, body, callback) {
    var options = {
        url: url,
        body: body,
        method: 'POST'
